...
class EngineeringOnePagePDF:
    """Professional one-page landscape PDF report without external packages."""
    ...
    def rect(self, x: float, y: float, w: float, h: float, fill: str | None = None, stroke: str | None = None, width: float = 0.7) -> None:
        if fill:
            self.fill(fill)
    ...
    def line(self, x1: float, y1: float, x2: float, y2: float, color: str = "#cbd5e1", width: float = 0.7) -> None:
        self.stroke(color)
        self.commands.append(f"{width:.2f} w {x1:.2f} {y1:.2f} m {x2:.2f} {y2:.2f} l S")

    def text(self, x: float, y: float, value: object, size: float = 8.0, color: str = "#0f172a", bold: bool = False, align: str = "left") -> None:
        text = self._escape(value)
        font = "F2" if bold else "F1"
        if align != "left":
            estimate = len(str(value)) * size * 0.48
            if align == "center":
                x -= estimate / 2
            elif align == "right":
                x -= estimate
        r, g, b = self._rgb(color)
        self.commands.append(f"BT /{font} {size:.2f} Tf {r:.3f} {g:.3f} {b:.3f} rg {x:.2f} {y:.2f} Td ({text}) Tj ET")

    def pill(self, x: float, y: float, w: float, h: float, text: str, fill: str, color: str = "#ffffff") -> None:
        self.rect(x, y, w, h, fill=fill, stroke=None)
        self.text(x + w / 2, y + h / 2 - 3, text, 8.0, color, True, "center")

    def section_title(self, x: float, y: float, text: str, w: float) -> None:
        self.rect(x, y - 16, w, 18, fill="#eff6ff", stroke="#bfdbfe")
        self.text(x + 8, y - 10, text.upper(), 7.5, "#1e3a8a", True)

    def key_value_table(self, x: float, y_top: float, w: float, title: str, rows: list[tuple[str, object]]) -> float:
        self.section_title(x, y_top, title, w)
        row_h = 15.0
        y = y_top - 18.0
        for index, (label, value) in enumerate(rows):
            fill = "#ffffff" if index % 2 == 0 else "#f8fafc"
            self.rect(x, y - row_h, w, row_h, fill=fill, stroke="#e2e8f0", width=0.4)
            self.text(x + 6, y - 10.5, label, 7.0, "#475569", False)
            self.text(x + w - 6, y - 10.5, value, 7.2, "#0f172a", True, "right")
            y -= row_h
        return y

    def small_schedule(self, x: float, y_top: float, w: float, title: str, headers: list[str], rows: list[list[object]]) -> float:
        self.section_title(x, y_top, title, w)
        row_h = 13.0
        y = y_top - 18.0
        col_w = w / len(headers)
        self.rect(x, y - row_h, w, row_h, fill="#1d4ed8", stroke="#1d4ed8")
        for i, header in enumerate(headers):
            self.text(x + i * col_w + col_w / 2, y - 9.0, header, 6.5, "#ffffff", True, "center")
        y -= row_h
        for index, row in enumerate(rows[:4]):
            fill = "#ffffff" if index % 2 == 0 else "#f8fafc"
            self.rect(x, y - row_h, w, row_h, fill=fill, stroke="#e2e8f0", width=0.35)
            for i, value in enumerate(row):
                self.text(x + i * col_w + col_w / 2, y - 9.0, value, 6.4, "#0f172a", False, "center")
            y -= row_h
        return y

    def results_table(self, x: float, y_top: float, w: float, rows: list[ResultRow]) -> None:
        title_h = 18.0
        self.section_title(x, y_top, "Uplift Pressure and Floor Safety Results", w)
        y = y_top - title_h
        headers = ["Pile", "Pt", "x", "d", "Base", "Mut.", "t corr.", "Slope", "Final", "h", "P", "Req t", "Prov t", "Status"]
        widths = [50, 25, 42, 42, 50, 45, 52, 45, 52, 42, 52, 52, 52, 45]
        scale = w / sum(widths)
        widths = [item * scale for item in widths]
        row_h = 15.5
        self.rect(x, y - row_h, w, row_h, fill="#0f2d4a", stroke="#0f2d4a")
        cx = x
        for header, col_w in zip(headers, widths):
            self.text(cx + col_w / 2, y - 10.4, header, 6.2, "#ffffff", True, "center")
            cx += col_w
        y -= row_h
        for index, row in enumerate(rows[:12]):
            ...
            if not row.safe:
                fill = "#fff1f2"
            self.rect(x, y - row_h, w, row_h, fill=fill, stroke="#dbeafe", width=0.35)
            values = [
                row.pile.replace("Pile ", "P"),
                row.point,
                fmt(row.x, 1),
                fmt(row.depth, 1),
                fmt(row.base_phi, 1),
                fmt(row.mutual, 1),
                fmt(row.thickness_correction, 1),
                fmt(row.slope, 1),
                fmt(row.corrected_phi, 1),
                fmt(row.uplift_head, 2),
                fmt(row.uplift_pressure, 1),
                fmt(row.required_thickness, 2),
                fmt(row.thickness, 2),
                "SAFE" if row.safe else "UNSAFE",
            ]
            cx = x
            for value, col_w in zip(values, widths):
                color = "#166534" if value == "SAFE" else "#b91c1c" if value == "UNSAFE" else "#0f172a"
                self.text(cx + col_w / 2, y - 10.4, value, 6.1, color, value in {"SAFE", "UNSAFE"}, "center")
                cx += col_w
            y -= row_h

    def pressure_profile(self, x: float, y_top: float, w: float, h: float, rows: list[ResultRow]) -> None:
        self.section_title(x, y_top, "Corrected Pressure Profile", w)
        y = y_top - 22
        chart_h = h - 26
        self.rect(x, y - chart_h, w, chart_h, fill="#ffffff", stroke="#dbeafe")
        if not rows:
            return
        max_phi = max(max(row.corrected_phi for row in rows), 1.0)
        step = w / max(len(rows), 1)
        points: list[tuple[float, float]] = []
        for idx, row in enumerate(rows):
            px = x + step * idx + step / 2
            py = y - chart_h + 8 + (row.corrected_phi / max_phi) * (chart_h - 18)
            points.append((px, py))
            self.rect(px - 1.7, py - 1.7, 3.4, 3.4, fill="#2563eb")
            self.text(px, y - chart_h + 1.5, f"{row.pile.replace('Pile ', 'P')}{row.point}", 5.0, "#64748b", False, "center")
        for idx in range(1, len(points)):
            x1, y1 = points[idx - 1]
            x2, y2 = points[idx]
            self.line(x1, y1, x2, y2, "#2563eb", 1.0)
        self.text(x + 5, y - 10, f"max phi = {fmt(max_phi, 1)}%", 6.0, "#475569")

    def save(self, path: str | Path, data: ProjectData, result: dict[str, object], design_status: str, recommendation: str, warnings: list[str]) -> None:
        margin = 26.0
        rows: list[ResultRow] = list(result["rows"])
        critical = result["critical"]
        max_pressure = max((row.uplift_pressure for row in rows), default=0.0)
        max_required = max((row.required_thickness for row in rows), default=0.0)
        status_color = "#16a34a" if design_status == "SAFE" else "#d97706" if design_status == "WARNING" else "#dc2626"

        self.rect(0, 0, self.width, self.height, fill="#ffffff")
        self.rect(0, self.height - 48, self.width, 48, fill="#0f2d4a")
        self.text(margin, self.height - 24, APP_TITLE, 16.0, "#ffffff", True)
        self.text(margin, self.height - 38, "One-page hydraulic foundation seepage and uplift pressure design summary", 7.2, "#dbeafe")
        self.pill(self.width - 130, self.height - 34, 100, 20, design_status, status_color)

        meta_y = self.height - 72
        self.rect(margin, meta_y - 28, self.width - 2 * margin, 28, fill="#f8fafc", stroke="#cbd5e1")
        meta = [
            ("Project", data.project_name),
            ("Structure", data.structure_type),
            ("Designer", data.designer or "-"),
            ("Date", data.design_date),
        ]
        meta_w = (self.width - 2 * margin) / len(meta)
        for i, (label, value) in enumerate(meta):
            x = margin + i * meta_w
            if i:
                self.line(x, meta_y - 28, x, meta_y, "#cbd5e1")
            self.text(x + 8, meta_y - 11, label.upper(), 5.8, "#64748b", True)
            self.text(x + 8, meta_y - 23, value, 7.5, "#0f172a", True)

        card_y = meta_y - 42
        card_gap = 8.0
        card_w = (self.width - 2 * margin - 4 * card_gap) / 5
        cards = [
            ("Seepage Head H", f"{fmt(float(result['H']), 3)} m", "#2563eb"),
            ("Max Uplift Pressure", f"{fmt(max_pressure, 2)} kN/m2", "#2563eb"),
            ("Critical Point", "-" if critical is None else f"{critical.pile} {critical.point}", "#7c3aed"),
            ("Exit Gradient GE", f"{fmt(float(result['exit_gradient']), 5)}", "#16a34a" if result["exit_safe"] else "#dc2626"),
            ("Required Max t", f"{fmt(max_required, 3)} m", "#0f766e"),
        ]
        for i, (label, value, color) in enumerate(cards):
            x = margin + i * (card_w + card_gap)
            self.rect(x, card_y - 34, card_w, 34, fill="#ffffff", stroke="#cbd5e1")
            self.text(x + 8, card_y - 12, label.upper(), 5.7, "#64748b", True)
            self.text(x + 8, card_y - 27, value, 10.2, color, True)

        info_top = card_y - 50
        left_w = 255.0
        mid_w = 245.0
        right_w = self.width - 2 * margin - left_w - mid_w - 16
        self.key_value_table(margin, info_top, left_w, "Hydraulic and Geometry Inputs", [
            ("Pond level", f"{fmt(data.pond_level)} m"),
            ("Tail water level", f"{fmt(data.tail_water_level)} m"),
            ("Safe exit gradient", fmt(data.safe_exit_gradient, 5)),
            ("Floor length b", f"{fmt(data.floor_length)} m"),
            ("Crest RL", f"{fmt(data.crest_level)} m"),
            ("Floor RL", f"{fmt(data.floor_level)} m"),
        ])
        node_rows = [[node.name, fmt(node.x, 2), fmt(node.thickness, 2)] for node in data.floor_nodes]
        self.small_schedule(margin + left_w + 8, info_top, mid_w, "Floor Thickness Nodes", ["Node", "x (m)", "t (m)"], node_rows)
        pile_rows = [[pile.name.replace("Pile ", "P"), fmt(pile.x, 2), fmt(pile.depth, 2), fmt(data.floor_thickness_at(pile.x), 2)] for pile in data.piles]
        self.small_schedule(margin + left_w + mid_w + 16, info_top, right_w, "Pile Schedule", ["Pile", "x", "d", "t"], pile_rows)

        profile_top = info_top - 104
        self.pressure_profile(margin, profile_top, 250, 76, rows)
        summary_x = margin + 258
        summary_w = self.width - margin - summary_x
        self.section_title(summary_x, profile_top, "Safety Summary and Recommendation", summary_w)
        self.rect(summary_x, profile_top - 78, summary_w, 58, fill="#ffffff", stroke="#dbeafe")
        self.text(summary_x + 10, profile_top - 34, f"Overall design status: {design_status}", 8.5, status_color, True)
        self.text(summary_x + 10, profile_top - 49, f"Exit-gradient status: {'SAFE' if result['exit_safe'] else 'UNSAFE'}  |  Unsafe pressure points: {result['unsafe_count']}", 7.2, "#0f172a")
        self.text(summary_x + 10, profile_top - 64, f"Recommendation: {recommendation}", 7.2, "#0f172a", True)

        table_top = profile_top - 92
        self.results_table(margin, table_top, self.width - 2 * margin, rows)

        footer = "Generated by Khosla Hydraulic Foundation Designer | Khosla Method of Independent Variables | Units: m and kN/m2"
        if warnings:
            footer += f" | Warnings: {len(warnings)}"
        self.line(margin, 24, self.width - margin, 24, "#cbd5e1", 0.6)
        self.text(margin, 13, footer, 6.2, "#64748b")
...
class KhoslaDesktopApp:
    ...
    def export_pdf(self) -> None:
        ...
        if not path:
            return
        try:
            pdf = EngineeringOnePagePDF()
            pdf.save(path, self.data, self.result, self.design_status(), self.primary_recommendation(), self.validation_warnings)
        except OSError as exc:
